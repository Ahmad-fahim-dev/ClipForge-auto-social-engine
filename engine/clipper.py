...
def detect_scenes(video_path, threshold=27.0):
    ...
    try:
        safe_path = video_path.replace("\\", "/").replace("'", r"'\''")
        filter_arg = f"movie='{safe_path}',select='gt(scene\\,0.3)'"
        cmd = [
            "ffprobe",
            "-v", "quiet",
            "-print_format", "json",
...
def process_video(video_id):
    ...
    logger.info("Processing video: %s", video['title'])
    ...
    logger.info("Created %d clips from: %s", created_clips, video['title'])

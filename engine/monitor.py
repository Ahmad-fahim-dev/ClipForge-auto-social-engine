...
def check_channel(channel_id_db, yt_channel_id):
    ...
    # Collect entries first (no DB while making network calls)
    entries_to_insert = []
    for entry in feed.entries:
        ...
        entries_to_insert.append((yt_video_id, title, video_url, thumbnail, published))

    # Now open DB connection, check duplicates, insert, close
    conn = get_db()
    for yt_video_id, title, video_url, thumbnail, published in entries_to_insert:
    ...
    conn.execute(
        ...
        (datetime.now(timezone.utc).isoformat(), channel_id_db)
    )
    conn.commit()
    conn.close()
...
def check_all_channels():
    ...
    if total_new > 0:
        ...
        logger.info("Monitor scan complete: %d new video(s) found", total_new)
    else:
        logger.info("Monitor scan complete: no new videos")

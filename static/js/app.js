...
async function api(url, method = 'GET', body = null) {
  try {
    ...
    if (!res.ok) {
      ...
      showToast(err.error || res.statusText, 'error');
      return null;
    }
...
function getSelectedIds(containerId) {
  return [...document.querySelectorAll(`#${containerId} input[type="checkbox"]:checked`)].map(c => parseInt(c.dataset.id));
}

function toggleAllCheckboxes(containerId, sourceCheckbox) {
  const checked = sourceCheckbox.checked;
  document.querySelectorAll(`#${containerId} input[type="checkbox"]`).forEach(c => c.checked = checked);
}
...
async function checkChannels() {
  const btn = document.querySelector('[data-action="check-channels"]') || event?.target;
  setLoading(btn, true);
...
async function downloadAll() {
  const btn = document.querySelector('[data-action="download-all"]') || event?.target;
  setLoading(btn, true);
  showToast('Downloading all pending videos...', 'info');
  const res = await api('/api/actions/download-all', 'POST');
  setLoading(btn, false);
  if (res) {
...
async function updateSchedulerStatus() {
  ...
  const sidebarIndicator = document.getElementById('sidebar-scheduler-indicator');
  if (sidebarIndicator) {
    sidebarIndicator.className = data.running ? 'status-dot running' : 'status-dot paused';
  }
}
